...
function initial(){
	...
	if(arg["redirect"]=="yes"){
		document.title = "Paper Locator - Redirect mode";
		goFreeWord();
		if(!canOpen)
			startSuggest();
	}else{
...
function submitReport(){
    ...
    //errorSubmit('./submiterror.html?'+citation);
}

/*
function errorSubmit(url){
	httpObj = new XMLHttpRequest();
	httpObj.onreadystatechange = submitCallback;
	if (httpObj){
...
//	reg.push(/\d{4}\D+(\d{1,3})\D+(\d{1,5})\s*-\s*\d{1,5}/);		//Nature, 2008, 23, 298-304.
    ...
    reg.push(/[12]\d{3}\D+(\d{1,3})\D+(\d{1,5})\D\d{1,5}/);
	reg.push(/[12]\d{3}\D+(\d{1,3})\D+(\d{1,5})/);
	//Followings are citation without a year.
	reg.push(/(\d{4}), (\d{1,5})\D+\d{1,5}/);
	reg.push(/(\d{1,3})(?:, |:)(\d{1,5})\D+\d{1,5}/);
	reg.push(/(\d{1,3})(?:, |:)(\d{6})/);
	reg.push(/(\d+).+?(\w+)\W*$/);								//PLoS Biol 9(2): e1000588. 
	var i;
